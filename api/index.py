...
# Simulating student data as provided
students_data = [
    {"name": "nk", "marks": 0},
    {"name": "gy5", "marks": 81},
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        names = query.split('&')
        names = [name.split('=')[1] for name in names if name.startswith('name=')]

        # Retrieve the marks for requested names
        result = {"marks": []}
        for name in names:
            # Find the student by name and get marks
            student = next((student for student in students_data if student['name'] == name), None)
